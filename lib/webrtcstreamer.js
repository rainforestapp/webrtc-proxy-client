...
export default class WebRtcStreamer {
  ...
  onKeyboardEvent({ down, code, keysym }) {
    ...
    if (down === undefined) {
      this.onKeyboardEvent({ down: true, code, keysym });
      this.onKeyboardEvent({ down: false, code, keysym });
      return;
    }
    console.log('sending key event with new system: ', keycode, keysym, down);
    this.onEvents([
      {
        keysym,
        keycode,
  ...
  onPasteText(text) {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      if (codepoint) {
        ...
        this.onKeyboardEvent({ code: codepoint, keysym });
      }
    }
  }
