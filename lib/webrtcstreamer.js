...
import { getQEMUExtendedKeyEvent, getTextKeyEvent } from './vnc';
...
export default class WebRtcStreamer {
  ...
  onKeyboardEvent({ down, code, keysym }) {
    let scancode = XtScancode[code];
    if (!scancode) {
      scancode = 0;
    }
    if (down === undefined) {
      this.onKeyboardEvent({ down, code, keysym });
      this.onKeyboardEvent({ down, code, keysym });
      return;
    }
    const buffer = getQEMUExtendedKeyEvent(keysym, down, scancode);
    this.onEvents([
      {
        buffer,
        isEvent: true
  ...
  onPasteText(text) {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      if (codepoint) {
        ...
        events.push({ keysym, down: true, isPress: true });
        events.push({ keysym, down: false, isPress: true });
      }
    }
