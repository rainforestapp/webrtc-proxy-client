...
export default class WebRtcStreamer {
  ...
  flushEvents() {
    ...
    this.sendEventTimeout = setTimeout(this.flushEvents.bind(null, this), 200);
  ...
  onEvents(events) {
    ...
    if (events.findIndex(ev => ev.isPress || ev.button) !== -1 || !sendEventTimeout) {
      this.flushEvents();
    }
  }
  ...
  setupEvents() {
    ...
    console.log('setting up webrtc element!', elem, this);
    // Mouse button state
    let buttonMask = 0;
